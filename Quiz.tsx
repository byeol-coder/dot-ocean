...
import { DISCOVERABLE, SPECIES, text, sizeScale, type Species } from '../data/species';
...
export function Quiz({ discovered, onClose }: { discovered: Set<string>; onClose: () => void }) {
  ...
  const newRound = useCallback(() => {
    ...
    setPicked(null);
    a.announce(ui.quizQuestion); a.speak(ui.quizQuestion);
  }, [pool, a, ui]);

  useEffect(() => { if (enough && !round) newRound(); }, [enough, round, newRound]);
    ...
    <div className="mode-screen">
      ...
      {!enough ? (
        ...
        <div className="quiz-body">
          <p className="quiz-q">{ui.quizQuestion}</p>
          ...
          <div className="quiz-options" role="group" aria-label={ui.quizQuestion}>
            {round.options.map((o) => {
              const t = text(o, lang);
          ...
          {picked && (
            <div className="quiz-reveal">
              ...
              <button className="btn-primary" onClick={() => { a.sfx('select'); newRound(); }}>{ui.quizNext} →</button>
            </div>
          )}
        </div>
      ) : null}
