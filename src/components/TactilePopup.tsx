...
import { pattern, dtmsToGrid } from '../engine/dotMatrix';
...
export function TactilePopup({ speciesKey, onDismiss, focusActive }: Props) {
  ...
  const dtmsHex = getDtmsPattern(speciesKey);
  const s = byKey[speciesKey];

  const dtmsGrid = useMemo(
    () => dtmsHex ? dtmsToGrid(dtmsHex) : undefined,
    [dtmsHex],
  );

  useEffect(() => {
    if (dtmsHex) {
      a.dotpad.renderHex(dtmsHex);
    } else if (s) {
      a.dotpad.render(pattern(speciesKey, sizeScale(s.sizeCm)));
    }
  ...
  return (
    <div className={cls} role="dialog" aria-modal={focusActive ? true : undefined} aria-label={`${t.name} ${ui.dotpadPreview}`}>
      ...
      <DotMatrix speciesKey={speciesKey} scale={sizeScale(s.sizeCm)} grid={dtmsGrid} ariaLabel={`${t.name}, ${t.tactile}`} />
